...
function buildDescriptionRows(): string[][] {
  ...
  rows.push(["表头字段", "预算值", "必填", "数字", "收入/成本/利润/现金流类为万元；ADR/RevPAR/客单价为元；占比为 %。", "180"]);
  rows.push(["表头字段", "单位", "选填", "文本", "建议与科目默认单位一致，导入时不强制校验。", "万元"]);
  ...
  rows.push([
    ...
    "—",
    "—",
    "1. 请勿修改「预算模板」sheet 表头名称。2. 收入/成本/利润类金额统一为万元。3. 未知科目将无法导入。4. 同一门店+年月+科目重复行以后写入为准。",
    ""
  ]);
