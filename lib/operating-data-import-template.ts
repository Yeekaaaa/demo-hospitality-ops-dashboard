...
const 客房收入说明 =
  "酒店客房相关收入，单位为万元；ADR、客房 RevPAR 等为系统计算字段，见本表【酒店房量指标】。";

const 填写注意全文 = `⚠ 填写注意：
1. 金额字段单位统一为万元，不要填写为元。
2. 房晚字段填写间夜数，建议为整数。
...
function buildDescriptionSheetModel(): { rows: string[][]; merges: Range[] } {
  ...
  rows.push([
    ...
    "营业收入",
    "revenue",
    "本账期门店总营业收入，单位为万元。例：300 表示 300 万元。",
    "万元"
  ]);
  rows.push([
    "经营结果",
  ...
  rows.push(["酒店房量指标", "客房收入", "room_revenue", 客房收入说明, "万元"]);
